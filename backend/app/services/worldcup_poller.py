...
async def poll_once(db: AsyncSession) -> float:
    ...
    if games:
        # Self-heal: link any knockout bracket slots the API has newly resolved
        # to real teams, then rebuild the oid map so their live scores/scorers
        # apply on this same tick (not just after a restart or full sync).
        if await link_resolved_knockouts(db, games, teams_by_seq):
            oid_map = await build_game_object_id_map(db)
